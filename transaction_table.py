...
def create_table():
    ...
    sql = "CREATE TABLE if not exists transaction_details (" \
          "transaction_id INTEGER NOT NULL auto_increment," \
          "account_number INTEGER NOT NULL," \
          "transaction_date DATE," \
          "transaction_type VARCHAR(30) NOT NULL," \
          "transaction_amount VARCHAR(20) NOT NULL," \
          "transaction_medium VARCHAR(20) NOT NULL," \
...
def insert(new_transaction_dict):
    ...
    insert_sql = "insert into transaction_details (account_number, transaction_date, transaction_type, transaction_amount, transaction_medium)" \
                 "values (%s, %s, %s, %s, %s)"
    ...
    try:
        db_cursor.execute(insert_sql, transaction_details)
        connection.commit()
...
def fetch_all():
    ...
    insert_sql = "select * from transaction_details"
    transactions = None
...
def fetch_one_with_transaction_id(transaction_id):
    ...
    select_sql = "select * from account where transaction_id = %d;"
    transaction = None

    try:
        db_cursor.execute(select_sql, (transaction_id,))
...
def update(updated_transaction_dict):
    ...
    update_sql = "update transaction_detail set (account_number = %s, transaction_date = %s, transaction_type = %s, " \
                 "transaction_amount = %s, account_status = %s) where transaction_id = %s"
    ...
    try:
        db_cursor.execute(update_sql, updated_transaction_details)
        connection.commit()
...
def delete_one_with_transaction_id(transaction_id):
    ...
    select_sql = "delete from transaction_details where transaction_id = %s"
